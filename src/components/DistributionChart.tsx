...
import { Holding, HoldingCategory, Instrument } from '../types';
...
export default function DistributionChart({ holdings, instruments }: DistributionChartProps) {
  const [hoveredCategory, setHoveredCategory] = useState<HoldingCategory | null>(null);

  // Group holdings by category using instruments lookup
  const categoryTotals = holdings.reduce<Record<HoldingCategory, { valuation: number; invested: number }>>(
    (acc, holding) => {
      const inst = instruments.find((i) => i.id === holding.instrumentId);
      const category: HoldingCategory = inst ? inst.category : 'other';
      if (!acc[category]) {
        acc[category] = { valuation: 0, invested: 0 };
      }
      acc[category].valuation += holding.currentValuation;
      acc[category].invested += holding.investedAmount;
      return acc;
    },
    {} as any
  );
